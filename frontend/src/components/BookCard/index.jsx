...
export default function BookCard({ book, compact = false, onEdit, onDelete }) {
  ...
  let coverUrl = `https://placehold.co/300x450/${bgColor}/${textColor}?text=${encodeURIComponent(book.title)}`
     ...
     return (
       <div className="group relative bg-white dark:bg-neutral-900 rounded-md hover:bg-slate-50 dark:hover:bg-neutral-800 transition-all duration-200 border border-slate-200 dark:border-neutral-800 hover:border-purple-300 dark:hover:border-purple-500/30 shadow-sm">
          ...
          <div className="flex gap-2 p-2">
             <img src={coverUrl} alt={book.title} className={`${compact ? 'w-16 h-24' : 'w-20 h-28'} object-cover rounded flex-shrink-0`} loading="lazy" decoding="async" />
             
             <div className="flex-1 min-w-0 flex flex-col gap-1.5">
                {/* Section 1: Title + Author */}
                <div className="flex-1">
                    <h4 className={`font-medium ${compact ? 'text-xs line-clamp-1' : 'text-sm line-clamp-2'} text-slate-800 dark:text-white leading-tight mb-0.5`} title={book.title}>{book.title}</h4>
                    <p className={`text-[10px] text-slate-500 dark:text-neutral-400 truncate`}>{book.author}</p>
                </div>
  ...
  return (
    <div className="group relative bg-white dark:bg-neutral-900 rounded-lg hover:bg-slate-50 dark:hover:bg-neutral-800 transition-all duration-200 border border-slate-200 dark:border-neutral-800 hover:border-purple-300 dark:hover:border-purple-500/30 flex h-40 shadow-sm">
      ...
      <div className="flex-1 p-2 flex flex-col gap-1 min-w-0">
         {/* Section 1: Title + Author */}
         <div className="h-14">
            <h3 className="font-semibold text-slate-800 dark:text-white text-xs leading-snug mb-0.5 line-clamp-2" title={book.title}>
               {book.title}
            </h3>
            <p className="text-[10px] text-slate-500 dark:text-neutral-400 truncate">{book.author}</p>
         </div>
         ...
         <div className="h-8 flex flex-col gap-0 text-[9px]">
            <span className="text-slate-500 dark:text-neutral-400 font-medium truncate" title={book.book_class}>
               {book.book_class}
            </span>
            <span className="text-slate-400 dark:text-neutral-400 truncate" title={book.category}>
               {book.category}
            </span>
         </div>
         ...
         <div className="h-6 flex items-center gap-1 flex-wrap">
            ...
            {book.score > 0 && (
               <div className="flex items-center gap-0.5 bg-purple-500/10 px-1.5 py-0.5 rounded border border-purple-500/20">
                  <span className="text-[10px] text-purple-600 dark:text-purple-400 font-bold">{book.score.toFixed(0)}</span>
                  <span className="text-[8px] text-slate-500 dark:text-neutral-500">Score</span>
               </div>
            )}
            ...
            {book.google_rating && (
               <div className="flex items-center gap-0.5 bg-amber-500/10 px-1.5 py-0.5 rounded border border-amber-500/20">
                  <Star size={8} fill="#fbbf24" className="text-amber-500 dark:text-amber-400" />
                  <span className="text-[10px] text-amber-600 dark:text-amber-400 font-medium">{book.google_rating.toFixed(1)}</span>
                  <span className="text-[8px] text-slate-500 dark:text-neutral-500">Google</span>
               </div>
            )}
            ...
            {book.rating > 0 && (
               <div className="flex items-center gap-0.5 bg-amber-500/10 px-1.5 py-0.5 rounded border border-amber-500/20">
                  <Star size={10} fill="#fbbf24" className="text-amber-500 dark:text-amber-400" />
                  <span className="text-[10px] text-amber-600 dark:text-amber-400 font-bold">{book.rating}</span>
               </div>
            )}
         </div>
