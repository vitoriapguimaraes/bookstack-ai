...
export default function BookCard({ book, compact = false, onEdit, onDelete }) {
          ...
          <div className="flex gap-2 p-2">
             ...
             <div className="flex-1 min-w-0 flex flex-col gap-2">
                ...
                <div className="flex-1">
                    <h4 className="font-medium text-sm text-slate-800 dark:text-white line-clamp-2 leading-tight mb-0.5" title={book.title}>{book.title}</h4>
                ...
                {book.rating > 0 && (
                   <div className="flex items-center gap-1 w-max bg-amber-500/10 px-2 py-0.5 rounded border border-amber-500/20">
                      <Star size={10} fill="#fbbf24" className="text-amber-500 dark:text-amber-400" />
                      <span className="text-xs text-amber-600 dark:text-amber-400 font-bold">{book.rating}</span>
                      <span className="text-[9px] text-slate-500 dark:text-neutral-500">Minha</span>
                   </div>
                )}
  ...
  return (
    <div className="group relative bg-white dark:bg-neutral-900 rounded-lg hover:bg-slate-50 dark:hover:bg-neutral-800 transition-all duration-200 border border-slate-200 dark:border-neutral-800 hover:border-purple-300 dark:hover:border-purple-500/30 flex h-56 shadow-sm">
      {/* Cover Image */}
      <div className="w-36 flex-shrink-0 bg-slate-100 dark:bg-neutral-950 relative overflow-hidden rounded-l-lg">
      ...
      <div className="flex-1 p-3 flex flex-col gap-3 min-w-0">
         ...
         <div className="h-20">
            <h3 className="font-semibold text-slate-800 dark:text-white text-sm leading-snug mb-1 line-clamp-3" title={book.title}>
               {book.title}
            </h3>
            <p className="text-xs text-slate-500 dark:text-neutral-400 truncate">{book.author}</p>
         </div>
         ...
         <div className="h-10 flex flex-col gap-0.5 text-[10px]">
            <span className="text-slate-500 dark:text-neutral-400 font-medium" title={book.book_class}>
               {book.book_class}
            </span>
            <span className="text-slate-400 dark:text-neutral-400" title={book.category}>
               {book.category}
            </span>
         </div>
         ...
         <div className="h-6 flex items-center gap-2 flex-wrap">
            ...
            {book.score > 0 && (
               <div className="flex items-center gap-1 bg-purple-500/10 px-2 py-0.5 rounded border border-purple-500/20">
                  <span className="text-xs text-purple-600 dark:text-purple-400 font-bold">{book.score.toFixed(0)}</span>
                  <span className="text-[9px] text-slate-500 dark:text-neutral-500">Score</span>
               </div>
            )}
            ...
            {book.google_rating && (
               <div className="flex items-center gap-1 bg-amber-500/10 px-2 py-0.5 rounded border border-amber-500/20">
                  <Star size={10} fill="#fbbf24" className="text-amber-500 dark:text-amber-400" />
                  <span className="text-xs text-amber-600 dark:text-amber-400 font-medium">{book.google_rating.toFixed(1)}</span>
                  <span className="text-[9px] text-slate-500 dark:text-neutral-500">Google</span>
               </div>
            )}
            ...
            {book.rating > 0 && (
               <div className="flex items-center gap-1 bg-amber-500/10 px-2 py-0.5 rounded border border-amber-500/20">
                  <Star size={12} fill="#fbbf24" className="text-amber-500 dark:text-amber-400" />
                  <span className="text-xs text-amber-600 dark:text-amber-400 font-bold">{book.rating}</span>
                  <span className="text-[9px] text-slate-500 dark:text-neutral-500">Minha</span>
               </div>
            )}
       ...
       {showMotivation && book.motivation && (
         <div className="absolute right-0 top-full mt-2 w-80 bg-white dark:bg-neutral-900 border border-slate-200 dark:border-blue-500/50 rounded-lg shadow-2xl p-4 z-30 animate-fade-in ring-1 ring-black/5">
           ...
           <p className="text-xs text-slate-600 dark:text-neutral-300 leading-relaxed max-h-48 overflow-y-auto">
             {book.motivation}
           </p>
           {/* Arrow pointing up to button */}
           <div className="absolute bottom-full right-2 w-0 h-0 border-l-8 border-l-transparent border-r-8 border-r-transparent border-b-8 border-b-white dark:border-b-blue-500/50"></div>
