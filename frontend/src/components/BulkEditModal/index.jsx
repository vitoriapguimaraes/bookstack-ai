            ...
            <div className="bg-neutral-900 border border-neutral-700 rounded-xl shadow-2xl w-full max-w-md overflow-hidden">
                <div className="bg-neutral-800/50 p-4 border-b border-neutral-700 flex justify-between items-center">
                    <h3 className="font-bold text-white flex items-center gap-2">
                        <span className="bg-purple-600 text-white text-xs px-2 py-0.5 rounded-full">{count}</span>
                        Edição em Massa
                    </h3>
                ...
                <div className="p-6 space-y-4">
                    <div className="bg-amber-500/10 border border-amber-500/20 rounded p-3 text-amber-200 text-xs flex gap-2 items-start">
                        ...
                        <p>Atenção: As alterações serão aplicadas a TODOS os {count} livros selecionados. Campos deixados em branco ("Não Alterar") serão mantidos como estão.</p>
                    </div>

                    {/* Class & Category */}
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-xs uppercase text-neutral-500 mb-1.5 font-bold">Nova Classe</label>
                            <select 
                                name="book_class" 
                                value={updates.book_class} 
                                onChange={(e) => setUpdates(prev => ({ ...prev, book_class: e.target.value, category: '' }))}
                        ...
                        <div>
                            <label className="block text-xs uppercase text-neutral-500 mb-1.5 font-bold">Nova Categoria</label>
                            <select 
                                name="category" 
                            ...
                            >
                                <option value="">(Não Alterar)</option>
                                {(CLASS_CATEGORIES[updates.book_class] || []).map(cat => (
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-xs uppercase text-neutral-500 mb-1.5 font-bold">Nova Disponibilidade</label>
                            <select 
                                name="availability" 
                        ...
                        <div>
                            <label className="block text-xs uppercase text-neutral-500 mb-1.5 font-bold">Nova Prioridade</label>
                            <select 
                                name="priority" 
                ...
                <div className="p-4 bg-neutral-950 border-t border-neutral-800 flex justify-end gap-2">
                    ...
                    <button onClick={handleSave} className="flex items-center gap-2 px-4 py-2 bg-purple-600 hover:bg-purple-500 text-white rounded font-medium transition-colors">
                        <Save size={16} />
                        Aplicar Alterações
                    </button>
                </div>
