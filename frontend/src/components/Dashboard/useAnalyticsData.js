...
export function useAnalyticsData(books) {
  return useMemo(() => {
    ...
    const avgRating =
      lidos.length > 0
        ? lidos.reduce((acc, b) => acc + (b.rating || 0), 0) /
          lidos.filter((b) => b.rating).length
        : 0;
    ...
    const avgScore =
      booksWithScore.length > 0
        ? booksWithScore.reduce((acc, b) => acc + (b.score || 0), 0) /
          booksWithScore.length
        : 0;
    ...
    const busiestYear =
      maxCount > 0 ? { years: busiestYearsList, count: maxCount } : null;
