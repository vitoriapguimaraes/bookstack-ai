...
export default function Analytics({ books }) {
  ...
  if (!books)
    return (
      <div className="p-10 text-center animate-pulse text-neutral-500">
        Carregando dados...
      </div>
    );
  if (!stats)
    return (
      <div className="p-10 text-center text-neutral-500">
        Nenhum livro encontrado para análise.
      </div>
    );
  ...
  const handleExportReport = async () => {
    ...
    try {
      ...
      link.download = `relatorio-leitura-${new Date()
        .toISOString()
        .slice(0, 10)}.png`;
  ...
  return (
    <div ref={reportRef} className="space-y-6 pb-10 animate-fade-in">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">
            Visualize indicadores e tendências do seu histórico de leitura.
          </p>
        </div>
        <button
          onClick={handleExportReport}
          disabled={isExporting}
          className={`flex items-center gap-2 px-4 py-2 bg-white dark:bg-neutral-800 text-slate-700 dark:text-white border border-slate-200 dark:border-neutral-700 hover:bg-slate-50 dark:hover:bg-neutral-700 rounded-lg text-sm font-medium transition-colors shadow-sm ${
      ...
      {stats.meta.unmappedCategories &&
        stats.meta.unmappedCategories.length > 0 && (
          <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700/50 p-4 rounded-lg flex gap-3 text-sm text-amber-800 dark:text-amber-200">
            ...
            <div>
              <p className="font-bold mb-1">
                Categorias sem Classe definida detectadas:
              </p>
              <p className="opacity-90 mb-2">
                As seguintes categorias não possuem uma classe associada (ou
                estão como "Outros") e aparecerão em cinza nos gráficos. Edite
                os livros no Mural ou na Tabela para corrigir.
              </p>
              <div className="flex flex-wrap gap-2">
                {stats.meta.unmappedCategories.map((cat) => (
                  <span
                    key={cat}
                    className="px-2 py-1 bg-amber-100 dark:bg-amber-800/50 rounded text-xs font-mono"
                  >
                    {cat}
                  </span>
                ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
        ...
        <KpiCard
          title="Lidos"
          value={stats.kpi.lidos}
          icon={BookOpen}
          color="text-pastel-green"
        />
        <KpiCard
          title="Na Fila"
          value={stats.kpi.aLer}
          icon={Layers}
      ...
      <div
        id="chart-timeline"
        className="relative bg-white dark:bg-neutral-900 border border-slate-200 dark:border-neutral-800 p-6 rounded-xl min-h-[500px] h-auto flex flex-col shadow-sm group"
      >
        <button
          onClick={() =>
            handleExportChart("chart-timeline", "historico-leitura")
          }
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4 flex-shrink-0">
          <div className="flex items-center gap-4">
            <h3 className="text-lg font-light text-slate-700 dark:text-neutral-200">
      ...
      <div id="chart-types" className="relative group">
        <DistributionChart
          ...
          chartType="bar"
          getColor={(entry) =>
            entry.name === "Técnico" ? "#d8b4fe" : "#fca5a5"
          }
        />
        <button
      ...
      <div id="chart-classes" className="relative group">
        ...
        <button
          onClick={() =>
            handleExportChart("chart-classes", "distribuicao-classes")
          }
          className="absolute top-4 right-4 p-2 text-slate-400 hover:text-slate-600 bg-white/50 hover:bg-white rounded-full opacity-0 group-hover:opacity-100 transition-all z-10"
        >
          <Download size={16} />
      ...
      <div id="chart-categories" className="relative group">
        ...
        <button
          onClick={() =>
            handleExportChart("chart-categories", "distribuicao-categorias")
          }
          className="absolute top-4 right-4 p-2 text-slate-400 hover:text-slate-600 bg-white/50 hover:bg-white rounded-full opacity-0 group-hover:opacity-100 transition-all z-10"
        >
          <Download size={16} />
