  ...
  return (
    <div className="w-full animate-fade-in space-y-6 pb-20">
      ...
      <div>
        ...
        {/* Reuse the same style as classes but simpler since it is a single list */}
        <AvailabilityManager
          options={classCategories.availability_options}
          setOptions={(newOptions) =>
            setClassCategories((prev) => ({
              ...prev,
              availability_options: newOptions,
            }))
          }
        />
      </div>
...
function AvailabilityManager({ options, setOptions }) {
  ...
  return (
    <div className="bg-white dark:bg-neutral-900 border border-slate-200 dark:border-neutral-800 rounded-xl overflow-hidden p-6">
      <div className="flex flex-wrap gap-2 mb-4">
        {safeOptions.map((opt) => (
          <div
            key={opt}
            className="flex items-center gap-2 px-3 py-1.5 bg-slate-100 dark:bg-neutral-800 rounded-full text-sm font-medium text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-neutral-700 animate-scale-in group"
          >
            {opt}
            <button
              onClick={() => handleRemove(opt)}
              className="p-0.5 rounded-full hover:bg-red-100 hover:text-red-500 dark:hover:bg-red-900/30 transition-colors opacity-0 group-hover:opacity-100"
            >
              <X size={14} />
            </button>
          </div>
