...
import { Plus, Trash2, Edit2, Check, X, Layers, Tag, Save, RotateCcw } from 'lucide-react'
...
export default function ListSettings() {
    ...
    const handleSave = async () => {
        ...
        try {
            const res = await api.get('/preferences/')
            await api.put('/preferences/', { ...res.data, class_categories: classCategories })
    ...
    const handleReset = () => {
        if (window.confirm("Isso restaurará os padrões de fábrica. Deseja continuar?")) {
            setClassCategories(DEFAULT_CATEGORIES)
        }
    }
    ...
    const removeClass = (cls) => {
        if (window.confirm(`Excluir a classe "${cls}" e todas as suas categorias?`)) {
    ...
    if (loading) return <div className="p-8 text-center text-slate-500 animate-pulse">Carregando listas...</div>

    return (
        <div className="w-full animate-fade-in space-y-6 pb-20">
            <div className="hidden md:block">
                <h2 className="text-2xl font-bold text-slate-800 dark:text-white">Listas e Campos</h2>
            ...
            <div className="space-y-4">
                {Object.entries(classCategories).map(([cls, cats]) => {
                    ...
                    return (
                        <div key={cls} className="bg-white dark:bg-neutral-900 border border-slate-200 dark:border-neutral-800 rounded-xl overflow-hidden transition-all group">
                            {/* Class Header */}
                                    ...
                                    {editingClass === cls ? (
                                        <div className="flex items-center gap-2 flex-1 max-w-sm">
                                            <input 
                                                ...
                                                onKeyDown={(e) => {
                                                    if(e.key === 'Enter') updateClassName(cls, e.target.value)
                            ...
                            <div className="p-4 bg-white dark:bg-neutral-900">
                                <div className="flex flex-wrap gap-2 mb-4">
                                    {cats.map((cat, idx) => {
                                        ...
                                        const catText = hslToString({ ...baseHSL, l: textL, s: 45 })
                                    ...
                                    <div className="flex items-center">
                                        {newCategory.class === cls ? (
                                            <div className="flex items-center gap-1 bg-slate-50 dark:bg-neutral-800 pl-3 pr-1 py-1 rounded-full border border-purple-500/30 animate-scale-in">
                                                <input 
                                                    ...
                                                    onKeyDown={(e) => {
                                                        ...
                                                        if(e.key === 'Escape') setNewCategory({ class: '', value: '' })
                                                    }}
                                                />
                                                <button 
                                                    onClick={() => addCat(cls)}
                                                ...
                                                <button 
                                                    onClick={() => setNewCategory({ class: '', value: '' })}
                                            ...
                                            <button 
                                                onClick={() => setNewCategory({ class: cls, value: '' })}
                ...
                <div className="border-2 border-dashed border-slate-200 dark:border-neutral-800 rounded-xl p-6 flex flex-col items-center justify-center text-center group hover:border-purple-500/50 transition-all">
                    <div className="w-12 h-12 rounded-full bg-slate-50 dark:bg-neutral-800 flex items-center justify-center mb-3 group-hover:bg-purple-50 dark:group-hover:bg-purple-900/20 transition-all">
                        <Plus className="text-slate-400 group-hover:text-purple-500" size={24} />
                    </div>
                    <h3 className="text-slate-800 dark:text-white font-bold mb-4">Nova Categoria de Livros</h3>
