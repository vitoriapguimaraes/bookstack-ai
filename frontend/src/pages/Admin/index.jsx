    ...
    return (
        <div className="w-full animate-fade-in space-y-6">
             ...
             <div className="hidden md:block">
                 <h2 className="text-2xl font-bold text-slate-800 dark:text-white">Painel Admin</h2>
            ...
            <div className="bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-700/30 p-4 rounded-lg flex gap-3">
               ...
               <p className="text-sm text-red-800 dark:text-red-200">
                  Acesso restrito a administradores. Aqui você pode visualizar e gerenciar os usuários cadastrados no sistema.
               </p>
            </div>
            ...
            <div className="bg-white dark:bg-neutral-900 rounded-lg border border-slate-200 dark:border-neutral-800 overflow-hidden">
                <div className="p-6 border-b border-slate-200 dark:border-neutral-800 flex justify-between items-center bg-white dark:bg-neutral-900">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                             <h3 className="text-base md:text-lg font-semibold text-slate-800 dark:text-white leading-tight">Usuários do Sistema</h3>
                             <p className="text-sm text-slate-500 dark:text-neutral-400">Listagem completa de registros</p>
                        </div>
                    </div>
                ...
                <div className="overflow-x-auto p-0">
                    <table className="w-full text-left text-sm">
                        <thead className="bg-slate-50 dark:bg-neutral-800/50 text-slate-500 dark:text-neutral-400 border-b border-slate-200 dark:border-neutral-800">
                            <tr>
                                <th className="px-6 py-3 text-xs font-semibold uppercase tracking-wider">ID (UUID)</th>
                                <th className="px-6 py-3 text-xs font-semibold uppercase tracking-wider">Email</th>
                                <th className="px-6 py-3 text-xs font-semibold uppercase tracking-wider">Função</th>
                                <th className="px-6 py-3 text-xs font-semibold uppercase tracking-wider">Data de Cadastro</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100 dark:divide-neutral-800">
                            {loading ? (
                                ...
                                users.map((u) => (
                                    <tr key={u.id} className="hover:bg-slate-50 dark:hover:bg-neutral-800/50 transition-colors group">
                                        <td className="px-6 py-4 font-mono text-xs text-slate-400 group-hover:text-slate-600 dark:group-hover:text-slate-300 transition-colors truncate max-w-[120px]" title={u.id}>
                                            {u.id}
                                        </td>
